#!/usr/bin/env python3
"""
MPC-based drone circle/lemniscate trajectory tracking with MuJoCo.

Architecture (cascade control):
  Outer loop (MPC, 50 Hz) → desired [thrust_N, roll_rad, pitch_rad, yaw_rate_rads]
...
def main():
    parser = argparse.ArgumentParser(description="MPC Drone Trajectory Tracking")
    ...
    print("=" * 60)
    print("  MPC Drone Trajectory Tracking  (cascade control)")
    print("=" * 60)
    print(f"  Trajectory : {args.trajectory}  r={args.radius}m  h={args.height}m  ω={args.omega}rad/s")
    print(f"  Duration   : {args.duration}s")
    print(f"  MPC horizon: {args.horizon}   outer dt={dt_ctrl}s   sim dt={dt_sim}s")
    print("=" * 60)
    ...
    # ---- Main loop ---------------------------------------------------------
    # outer_cmd is held constant between outer-loop solves
    ...
    for step in range(n_steps):
        ...
        # ---------- outer loop (MPC) every sim_steps_per_ctrl steps ----------
        if outer_counter == 0:
            ...
            if step % (50 * sim_steps_per_ctrl) == 0:
                ...
                print(f"  t={t_sim:6.2f}s  pos=({pos[0]:+.3f},{pos[1]:+.3f},{pos[2]:+.3f})"
                      f"  err={err:.4f}m  solve={t_solve*1e3:.1f}ms")
