#!/usr/bin/env python3
"""
Run MPPI-based drone circle trajectory tracking with MuJoCo visualization.
...
def main():
    parser = argparse.ArgumentParser(description="MPPI Drone Circle Tracking")
    ...
    parser.add_argument("--n-samples", type=int, default=256, help="Number of MPPI samples")
    parser.add_argument("--horizon", type=int, default=30, help="MPPI horizon")
    ...
    print("=" * 60)
    print("  MPPI Drone Trajectory Tracking")
    print("=" * 60)
    print(f"  Trajectory: {args.trajectory}")
    print(f"  Radius: {args.radius} m | Height: {args.height} m")
    print(f"  Angular speed: {args.omega} rad/s")
    print(f"  Duration: {args.duration} s")
    print(f"  MPPI: {args.n_samples} samples | horizon={args.horizon} | λ={args.temperature}")
    print("=" * 60)
    ...
    for step in range(n_ctrl_steps):
        ...
        # Get reference trajectory for MPPI horizon
        ...
        if step % 50 == 0:
            ...
            print(f"  t={t_sim:6.2f}s | pos=({pos[0]:+.3f}, {pos[1]:+.3f}, {pos[2]:+.3f}) | "
                  f"error={error:.4f}m | cost={info['best_cost']:.2f} | "
                  f"compute={t_compute*1000:.1f}ms")
